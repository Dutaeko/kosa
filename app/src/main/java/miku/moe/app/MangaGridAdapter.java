...
public class MangaGridAdapter extends BaseAdapter {
    ...
    @Override public View getView(int pos, View v, ViewGroup parent) {
        ...
        if (coverUrl.isEmpty()) {
            ...
            if (cachedCoverOnly) MangaImageLoader.loadCoverForSourceCachedOnly(h.image, coverUrl, sourceId, callback); else MangaImageLoader.loadCoverForSource(h.image, coverUrl, sourceId, callback);
        } else {
            h.image.animate().cancel();
            h.image.setAlpha(1f);
            setCoverLoading(h, false);
