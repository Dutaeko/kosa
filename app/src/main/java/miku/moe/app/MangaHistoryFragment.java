...
public class MangaHistoryFragment extends Fragment {
    ...
    private void refreshStoredHistoryData() {
        ...
        if (context == null || !isAdded() || !MangaSettingsManager.isMangaModeEnabled(context)) { setHistoryRefreshing(false); return; }
    ...
    private void finishHistoryRefresh(int token) {
        ...
        historyCoverReloadPending = false;
        coverHandler.removeCallbacksAndMessages(null);
        if (isAdded() && historyRecyclerView != null && historyAdapter != null) loadHistory();
        setHistoryRefreshing(false);
    }

    private void refreshStoredHistoryItem(ArrayList<MangaHistoryManager.Entry> items, int index, int token) {
        Context context = getContext();
        if (context == null || !isAdded() || token != historyRefreshToken || !MangaSettingsManager.isMangaModeEnabled(context)) { finishHistoryRefresh(token); return; }
        if (index >= items.size()) {
        ...
        api.detail(item.manga.slug, new KomikcastClient.Result<MangaPost>() {
            @Override public void onSuccess(MangaPost data, boolean hasNext) {
                ...
                if (callbackContext == null || !isAdded() || token != historyRefreshToken || !MangaSettingsManager.isMangaModeEnabled(callbackContext)) { finishHistoryRefresh(token); return; }
    ...
    private void bindHistoryCard(View itemView, MangaHistoryManager.Entry item) {
        ...
        MangaImageLoader.loadCoverForSourceCachedOnly(image, item.manga.coverImage, item.manga.getSourceId());
    ...
    private void applySelectionStyle(View card, String key) {
        ...
        View indicator = card.findViewById(R.id.selectionIndicatorTextView);
        if (indicator != null) indicator.setVisibility(selected ? View.VISIBLE : View.GONE);
        ...
        materialCard.setChecked(selected);
    ...
    private final class HistoryListAdapter extends RecyclerView.Adapter<RecyclerView.ViewHolder> {
        ...
        @Override public long getItemId(int position) {
            ...
            if (row.entry != null) return historyKey(row.entry).hashCode();
            return position;
        }
    ...
    private String historyRowsSignature(ArrayList<HistoryRow> rows) {
        ...
        for (HistoryRow row : rows) {
            ...
            if (row.type == HISTORY_ROW_HEADER) {
                ...
                String key = historyKey(row.entry);
                builder.append(key).append('|')
                        .append(selectedHistoryKeys.contains(key) ? 1 : 0).append('|')
