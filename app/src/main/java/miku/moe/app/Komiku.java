...
public class Komiku extends KomikcastClient {
    protected static String base() { return MangaSettingsManager.getSourceDomain(MangaSettingsManager.MANGA_SOURCE_KOMIKU); }
    ...
    @Override protected String sourceLabel() { return "Komiku"; }
            ...
            getDocument(url.toString(), new Result<Document>() {
                @Override public void onSuccess(Document document, boolean ignored) {
                    MangaCoroutines.io(() -> {
                        ...
                        } catch(Exception e) { MangaCoroutines.main(() -> cb.onError("Daftar Komiku gagal dibaca")); }
                    });
                }
    ...
    @Override public void genres(Result<ArrayList<GenreItem>> cb) {
        ...
        getDocument(base() + "/manga/", new Result<Document>() {
            @Override public void onSuccess(Document document, boolean ignored) {
                MangaCoroutines.io(() -> {
                    try {
                    ...
                    } catch(Exception e) { MangaCoroutines.main(() -> cb.onError("Genre Komiku gagal dibaca")); }
                });
            }
            @Override public void onError(String message) { cb.onError(message); }
        });
        ...
        getDocument(toAbsolute(slug), new Result<Document>() {
            @Override public void onSuccess(Document document, boolean ignored) {
                MangaCoroutines.io(() -> {
                    ...
                    } catch(Exception e) { MangaCoroutines.main(() -> cb.onError("Detail Komiku gagal dibaca")); }
                });
            }
            @Override public void onError(String message) { cb.onError(message); }
        });
            ...
            @Override public void onSuccess(Document document, boolean ignored) {
                MangaCoroutines.io(() -> {
                    try {
                        ...
                        MangaCoroutines.main(() -> cb.onSuccess(out, false));
                    } catch(Exception e) { MangaCoroutines.main(() -> cb.onError("Chapter Komiku gagal dibaca")); }
                });
            }
            @Override public void onError(String message) { cb.onError(message); }
        });
    ...
    @Override public void pages(String slug, float index, Result<ArrayList<String>> cb) {
        ...
        chapters(slug, new Result<ArrayList<MangaChapter>>() {
            @Override public void onSuccess(ArrayList<MangaChapter> chapters, boolean hasNext) {
                ...
                if (chapter == null || chapter.slug == null || chapter.slug.isEmpty()) { cb.onError("Chapter Komiku tidak ditemukan"); return; }
                    ...
                    @Override public void onSuccess(Document document, boolean ignored) {
                        MangaCoroutines.io(() -> {
                            try {
                                ...
                                MangaCoroutines.main(() -> cb.onSuccess(out, false));
                            } catch(Exception e) { MangaCoroutines.main(() -> cb.onError("Halaman Komiku gagal dibaca")); }
                        });
                    }
                    @Override public void onError(String message) { cb.onError(message); }
                });
    ...
    private HttpUrl buildListUrl(int page, String sort, String query, String genre) throws Exception {
        ...
        String path = searching && safePage > 1 ? base() + "/page/" + safePage + "/" : base() + "/manga/";
        ...
        else if ("ongoing".equals(s)) builder.addQueryParameter("status", "ongoing");
        else if ("completed".equals(s) || "complete".equals(s)) builder.addQueryParameter("status", "completed");
        else builder.addQueryParameter("order", "update");
        return builder.build();
    }
    ...
    private MangaPost parseListPost(Element element) {
        ...
        MangaPost post = new MangaPost(slug, title, cover, "", "", "", "", type, "", "").withSource(MangaSettingsManager.MANGA_SOURCE_KOMIKU, "Komiku");
    ...
    private MangaPost parseDetail(String slug, Document document) {
        ...
        MangaPost post = new MangaPost(slug, title, cover, author, status, synopsis, genre, type, "", "").withSource(MangaSettingsManager.MANGA_SOURCE_KOMIKU, "Komiku");
    ...
    private void getDocument(String url, Result<Document> cb) {
        ...
        CloudflareHelper.enqueue(client, req, sourceLabel(), new Callback() {
            ...
            @Override public void onResponse(Call call, Response response) throws IOException {
                ...
                catch(Exception e) { MAIN.post(() -> cb.onError("Data Komiku gagal dibaca")); }
            }
        });
    }
    ...
    private static String extractGenreFilter(String raw) {
        if (raw == null) return "";
        ...
        for (String part : parts) {
            ...
            if (!value.toLowerCase(Locale.ROOT).startsWith("type:")) return normalizeGenreValue(value);
        }
        return "";
    }

    private static String extractTypeFilter(String raw) {
        if (raw == null) return "";
        String[] parts = raw.split("\\|");
        for (String part : parts) {
            if (part == null) continue;
