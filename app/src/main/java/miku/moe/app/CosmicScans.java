...
public class CosmicScans extends KomikcastClient {
        ...
        getDocument(base() + "/manga/?order=update", new Result<Document>() {
            @Override public void onSuccess(Document document, boolean ignored) {
                MangaCoroutines.io(() -> {
                    ...
                    } catch(Exception e) { MangaCoroutines.main(() -> cb.onError("Genre CosmicScans gagal dibaca")); }
                });
            }
            @Override public void onError(String message) { cb.onError(message); }
        });
            ...
            @Override public void onSuccess(Document document, boolean ignored) {
                MangaCoroutines.io(() -> {
                    try {
                        ...
                        MangaCoroutines.main(() -> cb.onSuccess(out, false));
                    } catch(Exception e) { MangaCoroutines.main(() -> cb.onError("Chapter CosmicScans gagal dibaca")); }
                });
            }
            @Override public void onError(String message) { cb.onError(message); }
        });
    ...
    @Override public void pages(String slug, float index, Result<ArrayList<String>> cb) {
        ...
        chapters(slug, new Result<ArrayList<MangaChapter>>() {
            @Override public void onSuccess(ArrayList<MangaChapter> chapters, boolean hasNext) {
                ...
                if (chapter == null || chapter.slug == null || chapter.slug.isEmpty()) { cb.onError("Chapter CosmicScans tidak ditemukan"); return; }
    ...
    private HttpUrl buildListUrl(int page, String sort, String query, String genre) throws Exception {
        ...
        if ("project".equals(s) || "projects".equals(s)) {
            String path = safePage > 1 ? base() + "/projects/page/" + safePage + "/" : base() + "/projects/";
            return HttpUrl.parse(path).newBuilder().build();
        }
        ...
        if ("ongoing".equals(s)) {
            builder.addQueryParameter("status", "ongoing");
            builder.addQueryParameter("order", "");
            return builder.build();
        }
        String order = "update";
    ...
    private MangaPost parseListPost(Element element, String fallbackType) {
        ...
        MangaPost post = new MangaPost(slug, title, cover, "", "", "", "", type, "", "").withSource(MangaSettingsManager.MANGA_SOURCE_COSMICSCANS, "CosmicScans");
    ...
    private MangaPost parseDetail(String slug, Document document) {
        String title = text(document.selectFirst(".bigcontent .infox h1, .seriestuhead h1.entry-title, h1.entry-title, h1"));
        String author = firstNonEmpty(tableValue(document, "Author"), imptValue(document, "Author"));
        String status = firstNonEmpty(tableValue(document, "Status"), imptValue(document, "Status"));
        String type = firstNonEmpty(tableValue(document, "Type"), imptValue(document, "Type"));
        String genre = parseDetailGenres(document);
        String synopsis = text(document.selectFirst(".entry-content-single p, .seriestuhead .entry-content-single p"));
        ...
        String cover = image(document.selectFirst(".bigcontent .thumb img, .seriestucont .thumb img, .thumb img"));
    ...
    private static String parseDetailGenres(Document document) {
        LinkedHashSet<String> genres = new LinkedHashSet<>();
        collectGenreTexts(genres, document.select(".infox .wd-full .mgen a, .wd-full .mgen a, .mgen a, .seriestugenre a"));
        if (genres.isEmpty()) collectGenreTexts(genres, document.select("a[href*=/genres/]"));
        if (genres.isEmpty()) collectGenreTexts(genres, document.select("a[href*=\"/genre/\"]"));
    ...
    private static void collectGenreTexts(LinkedHashSet<String> genres, Elements elements) {
        if (genres == null || elements == null) return;
        for (Element e : elements) {
            String name = text(e);
            if (name.isEmpty()) continue;
            String lower = name.toLowerCase(Locale.ROOT);
            if (lower.contains("genre") || lower.contains("home") || lower.contains("bookmark")) continue;
            genres.add(name);
        }
    }
    ...
    private ArrayList<MangaChapter> parseChapters(Document document) {
        ...
        LinkedHashSet<String> seen = new LinkedHashSet<>();
        for (Element element : document.select(".eplister li, .bxcl ul li")) {
            ...
            String date = text(urlElement.selectFirst(".chapterdate, .date"));
    ...
    private static String image(Element element) {
        ...
        String[] attrs = {"abs:data-wpfc-original-src", "abs:data-src", "abs:data-lazy-src", "abs:src", "data-wpfc-original-src", "data-src", "data-lazy-src", "src"};
    ...
    private static String imptValue(Document document, String key) {
        for (Element row : document.select(".imptdt")) {
            String all = text(row);
            if (all.toLowerCase(Locale.ROOT).startsWith(key.toLowerCase(Locale.ROOT))) return all.substring(Math.min(key.length(), all.length())).trim();
        }
        return "";
    }
