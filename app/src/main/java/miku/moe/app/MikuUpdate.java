...
public class MikuUpdate extends AppCompatActivity {
    ...
    private void checkAt(ArrayList<MangaPost> snapshot, int index, boolean changed, int found) {
        ...
        MangaRepository.INSTANCE.detailFuture(oldPost.getSourceId(), oldPost.slug).whenComplete((fresh, detailError) -> {
            if (detailError != null || fresh == null) {
                resolveUpdateType(oldPost, oldPost, sourceApi, resolvedOld -> {
                    boolean itemChanged = replaceFavorite(oldPost, resolvedOld);
                    handler.postDelayed(() -> checkAt(snapshot, index + 1, changed || itemChanged, found), 100);
                });
                return;
            }
            MangaPost merged = mergeFavorite(oldPost, fresh);
            resolveUpdateType(oldPost, merged, sourceApi, resolved -> MangaRepository.INSTANCE.chaptersFuture(oldPost.getSourceId(), oldPost.slug).whenComplete((chapters, chapterError) -> {
