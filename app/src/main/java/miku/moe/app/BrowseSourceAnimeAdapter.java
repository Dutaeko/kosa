...
public class BrowseSourceAnimeAdapter extends BaseAdapter {
    ...
    private View getGridView(AnimePost post, View convertView, ViewGroup parent) {
        ...
        if (holder.imageShadow != null) holder.imageShadow.setVisibility(View.VISIBLE);
    ...
    private void bindCover(ImageView image, AnimePost post, Object holder) {
        ...
        if (!key.equals(current) || image.getDrawable() == null) {
            ...
            Glide.with(context).load(cover).centerCrop().into(image);
        } else {
            image.animate().cancel();
            image.setAlpha(1f);
        }
    ...
    private String episodeText(AnimePost post) {
        if (post == null) return "";
        String value = firstUseful(post.episodeCount, post.channelName);
        if (value.isEmpty()) return "";
        return value.toLowerCase().contains("episode") ? value : "Episode " + value;
    }
