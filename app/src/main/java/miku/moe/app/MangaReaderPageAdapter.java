...
final class MangaReaderPageAdapter extends RecyclerView.Adapter<RecyclerView.ViewHolder> {
    ...
    void preloadAround(Context context, int center) {
        ...
        int end = Math.min(entries.size() - 1, safeCenter + 5);
        ArrayList<String> urls = new ArrayList<>();
        for (int i = start; i <= end; i++) {
            Entry entry = entries.get(i);
            if (entry.type == TYPE_PAGE && entry.url != null && !entry.url.trim().isEmpty()) urls.add(entry.url);
        }
        if (urls.isEmpty()) return;
        Context app = context.getApplicationContext();
        MangaCoroutines.io(() -> {
            for (String url : urls) MangaImageLoader.preload(app, url, sourceId);
        });
    }
